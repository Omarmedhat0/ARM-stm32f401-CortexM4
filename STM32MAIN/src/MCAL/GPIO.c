...
Error_enumStatus_t GPIO_InitPin(GPIO_Config_t *Loc_GPIOElement)
{
    ...
    /* Local Variable to store new values*/
    uint32_t Loc_Temp_Value = 0;
    ...
    else if(    ((Loc_GPIOElement->Port) != GPIO_PORTA) && ((Loc_GPIOElement->Port) != GPIO_PORTB) &&
                ((Loc_GPIOElement->Port) != GPIO_PORTC) && ((Loc_GPIOElement->Port) != GPIO_PORTD) &&
                ((Loc_GPIOElement->Port) != GPIO_PORTE) && ((Loc_GPIOElement->Port) != GPIO_PORTH)
            )
    {
        Loc_enumReturnStatus = Status_enumWrongInput;
    }
    ...
    else
    {
        ...
        Loc_Temp_Value = (Loc_GPIOElement->Mood) & GPIO_MODE_MASK; ;
        Loc_MODER |= Loc_Temp_Value << ((Loc_GPIOElement->Pin) * GPIO_GROUP_OF_2_BITS);
        /*Assign the next bit in Loc_OutType_Value to indicate the status of the output type if it was
         Open drain or Push-Pull */   
        Loc_Temp_Value = 0;
        Loc_Temp_Value = ((Loc_GPIOElement->Mood) & GPIO_OUT_TYPE_MASK) >> GPIO_OUT_TYPE_SHIFT; 
        Loc_OTYPER |= (Loc_Temp_Value << ((Loc_GPIOElement->Pin)));
        /*Assign the next 2 bits in Loc_PuLL_Value to indicate the status of the PULL type if it was
         Floating , Pull Up or Pull Down   */
        Loc_Temp_Value = 0;
        Loc_Temp_Value = ((Loc_GPIOElement->Mood) & GPIO_PULL_TYPE_MASK) >> GPIO_PULL_TYPE_SHIFT; 
        Loc_PUPDR |= (Loc_Temp_Value << ((Loc_GPIOElement->Pin) * GPIO_GROUP_OF_2_BITS));

        /*Assign the required  speed */
