...
Error_enumStatus_t HUART_Init(void)
{
    ...
    uint8_t Loc_idx;
    GPIO_Config_t UART_PINS[UART_PINS_NUM] = {
        [TX_ID] = {.Mood = GPIO_AF_PP, .Speed = GPIO_HIGH_SPEED},
        [RX_ID] = {.Mood = GPIO_AF_PP, .Speed = GPIO_HIGH_SPEED}};
    ...
    for (Loc_idx = 0; Loc_idx < _USART_Num; Loc_idx++)
    {
        ...
        GPIO_CFG_AlternativeFunction(UART_PINS[TX_ID].Port, UART_PINS[TX_ID].Pin, HUARTS[Loc_idx].TX_AF_ID);
        GPIO_CFG_AlternativeFunction(UART_PINS[RX_ID].Port, UART_PINS[RX_ID].Pin, HUARTS[Loc_idx].TX_AF_ID);
